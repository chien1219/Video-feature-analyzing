...
public class VideoStruc {
    ...
    public void readAndextractSound() throws FileNotFoundException, IOException{
        String soundFileName;
        if(type == 0)
            soundFileName= FXMLController.DATABASE_DIR + videoname + "/" + videoname + ".wav";
        else
            soundFileName= "query_videos/" + videoname + "/" + videoname  + ".wav";
        try {
    ...
    public void readAndextractVideo() {
        for(int i = 0;i < framenum;i++) {
            ...
            String imgpath;
            if(type == 0)
                imgpath= FXMLController.DATABASE_DIR + videoname + "/" + videoname + num + ".rgb";
            else
                imgpath= "query_videos/" + videoname + "/" + videoname + num + ".rgb";
            File file = new File(imgpath);
            if (!file.exists())
            {
                if(type == 0)
                    imgpath= FXMLController.DATABASE_DIR + videoname + "/" + videoname + "_" + num + ".rgb";
                else
                    imgpath= "query_videos/" + videoname + "/" + videoname + "_" + num + ".rgb";
            }
